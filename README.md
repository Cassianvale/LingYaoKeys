...
- [x]支持顺序/按压热键模式切换
- [x]支持侧键和滚轮触发
- [x]支持鼠标移动至对应坐标，可进行坐标录入和编辑
- [x]每个按键及坐标设有独立间隔
- [x]支持窗口句柄嗅探，开启后仅对应窗口可触发热键
- [x]语音提醒开关，音量大小设置，自定义按键开启/按键关闭语音
- [x]正常/降低卡位模式切换开关
- [x]按键和坐标支持拖拽排序
- [x]浮窗置顶显示按键启动/关闭/禁用状态
- [x]支持输入法切换开关，开启后触发热键自动切换ENG输出
- [x]支持配置导出/导入、联网更新、调试模式等功能
...
> [!IMPORTANT]
> 经过长时间的测试并且结合其他按键的测试结果，按键速度每秒高于两三百的话会导致按键响应延迟或者造成卡位移(
> 系统io输出的响应时间+游戏客户端响应时间，看自己cpu性能)
> ，原因可能是windows的消息机制导致的，所以玩游戏的话按键速度，不需要太快。根据测试结果我取了一个合适的区间，所以就加入了针对游戏进行优化的降低卡位功能  
...
2. **系统要求**
    - 支持 Windows 10/11 (x86/x64)，Windows 7 未经过测试，可能导致无法预知的问题
    - 需要管理员权限运行
...
- **请使用者在使用本项目时遵守相关法律法规，不要将本项目用于任何商业及非法用途。如有违反，一切后果由使用者自负。
  同时，使用者应该自行承担因使用本项目而带来的风险和责任。本项目开发者不对本项目所提供的服务和内容做出任何保证**
